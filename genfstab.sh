...
# handle swap devices
{
  ...
  while read -r device type _ _ prio; do
    ...
    # skip devices not part of the prefix unless it's zram
    if [[ $device != /dev/zram* ]] && [[ $device != "$prefixfilter"* ]]; then
      continue
    fi

    # optionally skip zram if not meant to be persistent
    if [[ $device == /dev/zram* ]]; then
      echo "# Skipping ZRAM swap device: $device"
      continue
    fi

    # append priority if set
    if (( prio >= 0 )); then
      options+=,pri=$prio
    fi
